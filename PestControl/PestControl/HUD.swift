...
//  PestControl
//
//  Created by Eugene  Mekhedov on 8/5/19.
...
class HUD: SKNode{
  ...
  required init?(coder aDecoder: NSCoder) {
    ...
    bugCountLabel = childNode(withName: "BugsCounter") as? SKLabelNode
  ...
  func addCountBugs(bugs: Int){
    guard let scene = scene else { return }
    let position = CGPoint(x: scene.frame.size.width / 2 - 100,
                           y: scene.frame.size.height/2 - 10)
    add(message: "BugsCounter", position: position, fontSize: 24)
    bugCountLabel = childNode(withName: "BugsCounter") as? SKLabelNode
    bugCountLabel?.verticalAlignmentMode = .top
    bugCountLabel?.fontName = "Menlo"
    updateBugCounter(bugsCount: bugs)
  }
  
  func updateBugCounter(bugsCount: Int){
    bugCountLabel?.text = "Bugs: \(bugsCount)"
  }
